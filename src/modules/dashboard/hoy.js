...
export function resumenHoy(vivo, { hoy, locales = [] }) {
  ...
  const sumar = dia => {
    const filas = fuentes.map(l => !l?.error && l?.dias?.find(f => f.dia === dia));
    const completas = filas.length > 0 && filas.every(f => f && numero(f.ventas) !== null);
