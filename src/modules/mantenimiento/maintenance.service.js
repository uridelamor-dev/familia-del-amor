...
// (con get/all/run; run devuelve { lastID, changes }), usuario autenticado, estado de
// PERMISOS_V2 (`enabled`), parámetros de la operación y `now` inyectable.
//
...
export async function updateMaintenanceIssueStatus(x, user, id, { estado } = {}, { enabled = false, now } = {}) {
  ...
  const r = await x.run(`UPDATE maintenance_issues SET estado = ? WHERE id = ?`, [estado, idNum]);
  if (!r || r.changes === 0) return { code: "NOT_FOUND" }; // confirmar que se actualizó una fila
  return { code: "OK" };
}
