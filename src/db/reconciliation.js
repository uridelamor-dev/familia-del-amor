...
// Iteración 2 — PostgreSQL nativo. Solo lectura y diagnóstico. La normalización se usa
// ÚNICAMENTE para comparar; NUNCA modifica los strings originales. Los nombres de tabla se
// obtienen de information_schema (no de input externo) y se validan con un patrón de
// identificador antes de usarlos entre comillas dobles.
...
// Detecta tablas con columna `local` usando el catálogo del sistema de PostgreSQL
// (information_schema, esquema `public`). Los nombres se validan con IDENT_RE.
...
export async function collectLocalValues(x, tableNames) {
  ...
  for (const t of tableNames) {
    ...
    const rows = await x.all(
      `SELECT local AS local, COUNT(*)::int AS n FROM "${t}" WHERE local IS NOT NULL AND TRIM(local) <> '' GROUP BY local`, []
    );
