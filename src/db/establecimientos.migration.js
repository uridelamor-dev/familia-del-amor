// Migración aditiva del modelo de establecimientos (Iteración 2).
// - Solo crea tablas/índices nuevos; NO toca tablas ni columnas existentes.
// - No se auto-ejecuta al arrancar el servidor; se invoca deliberadamente desde el runner.
// - El backfill + grandfather + migration_state se ejecutan de forma ATÓMICA (una transacción).
// Recibe siempre un `x` con { run, get, all } (misma forma que tests/helpers/db.js).
...
export async function seedCatalogo(x, catalogo = CATALOGO_CANONICO) {
  ...
  for (const nombre of catalogo) {
    await x.run(
      `INSERT OR IGNORE INTO establecimientos (nombre, local_text, empresa_id, activo, creado_en)
       VALUES (?, ?, NULL, 1, ?)`, [nombre, nombre, now]
    );
  }
}
...
export async function applyBackfill(x, candidates) {
  ...
  for (const c of candidates) {
    const r = await x.run(
      `INSERT OR IGNORE INTO user_locations (usuario_id, establecimiento_id, activo, desde, hasta, creado_en)
       VALUES (?, ?, 1, ?, NULL, ?)`, [c.usuario_id, c.establecimiento_id, now, now]
    );
    if (r && r.changes) inserted += r.changes;
  }
  return inserted;
}
...
export async function applyGrandfather(x, approvedIds) {
  ...
  for (const id of approvedIds) {
    ...
    await x.run(
      `INSERT OR IGNORE INTO legacy_access (usuario_id, motivo, concedido_en, activo, revocar_en)
       VALUES (?, 'cuenta existente en la migración', ?, 1, NULL)`, [id, now]
    );
    applied.push(id);
  }
...
// Cualquier fallo revierte TODO (sin migration_state, sin asignaciones ni legacy parciales).
export async function migrarBackfill(x, { approvedLegacyIds } = {}) {
  ...
  try {
    ...
    const usersCount = (await x.get("SELECT COUNT(*) AS n FROM users", [])).n;
...
export const CREATED_INDEXES = ["idx_user_locations_usuario", "idx_user_locations_estab"];
