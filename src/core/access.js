...
// PostgreSQL: código SQLSTATE 42P01 (undefined_table) o mensaje "relation ... does not exist".
function isMissingTableError(e) {
  if (e && e.code === "42P01") return true;
  const m = e && (e.message || String(e));
  return typeof m === "string" && /relation .* does not exist|does not exist/i.test(m);
}
    ...
    rows = await x.all(
      `SELECT ul.establecimiento_id AS eid, e.local_text AS lt
        ...
        WHERE ul.usuario_id = ?
          AND ul.activo = 1
          AND e.activo = 1
          AND (ul.desde IS NULL OR ul.desde::date <= ?::date)
          AND (ul.hasta IS NULL OR ul.hasta::date >= ?::date)`,
      [usuarioId, ref, ref]
    );
  } catch (e) {
    ...
    const row = await x.get(
      `SELECT 1 AS ok FROM legacy_access
        WHERE usuario_id = ? AND activo = 1
          AND (revocar_en IS NULL OR revocar_en::date >= ?::date)
        LIMIT 1`,
      [usuarioId, ref]
    );
    return !!row;
