...
function construirPdf(objetos, { comprimirContenido = false } = {}) {
...
function contenidoDe(items, fuente = "/F1") {
 ...
 */
function facturaConTexto(items, { fuente, extraObjetos = [], comprimir = false } = {}) {
...
describe("facturas.js manda la capa junto al documento", () => {
  ...
  test("las instrucciones de extracción siguen yendo las últimas", () => {
    assert.ok(cuerpo.lastIndexOf("PROMPT_EXTRACCION") > cuerpo.indexOf("bloqueTextoParaClaude"));
  });
});
